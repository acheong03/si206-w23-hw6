...
# Who you worked with: Adena Cartsonis    #
###########################################
...
def cache_all_pages(people_url, filename):
    ...
    page_num = 1
    while True:
        if 'page' + str(page_num) not in cache:
            params = {'page' : page_num}
            res = get_swapi_info(people_url, params=params)
            if res is not None:
                thing = 'page ' + str(page_num)
                cache[thing] = res['results']
                if res['next'] is None:
                    break
                else:
                    page_num += 1
